...
class Website_model extends CI_Model{
    ...
    public function get_search_data_report($data){
        ...
        return  $this->db->select('*, @curRank := @curRank + 1 AS number', FALSE)
                        ->get('v_report p, (SELECT @curRank := 0) r')->result_array();
    }
    ...
    public function get_report_data($limit, $start, $datas){
        ...
        return  $this->db->select('*, @curRank := @curRank + 1 AS number', FALSE)
                        ->get('v_report p, (SELECT @curRank := 0) r', $limit, $start)->result_array();
    }
